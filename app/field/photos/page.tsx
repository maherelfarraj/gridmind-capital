...
export default function PhotosPage() {
  ...
  async function onPick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    else { toast({ title: 'Photo uploaded', variant: 'success' }); mutate() }
  ...
  async function remove(p: FieldPhoto) {
    ...
    if (res.error) toast({ title: res.error, variant: 'danger' })
    else { toast({ title: 'Photo removed', variant: 'success' }); mutate() }
  }
    ...
    <div className="px-4 py-4">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-lg font-semibold text-foreground flex items-center gap-2">
          <ImageIcon className="size-5 text-primary" /> Site Photos
        </h1>
        <button
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border px-6 py-14 text-center">
          <ImageIcon className="size-8 text-muted-foreground/50 mx-auto mb-3" />
          <p className="text-sm font-medium text-foreground">No photos yet</p>
          <p className="text-xs text-muted-foreground mt-1">Tap Capture to add a site photo from your camera.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-2.5">
          {photos.map((p) => (
            <div key={p.id} className="relative aspect-square rounded-xl overflow-hidden border border-border bg-muted">
              ...
              <img src={p.url || '/placeholder.svg'} alt={p.caption ?? 'Site photo'} className="w-full h-full object-cover" />
              {p.caption && (
              ...
              <button
                onClick={() => remove(p)}
                className="absolute top-1.5 right-1.5 rounded-full bg-black/55 p-1.5"
                aria-label="Delete photo"
              ><Trash2 className="size-3.5 text-white" /></button>
            </div>
          ))}
        </div>
      )}
