-- GM-G3: decide_gate_approval v5 -- prohibit requester self-approval
--
-- A production browser smoke test exposed a SEGREGATION-OF-DUTIES defect: the
-- requester of a gate approval could decide it themselves because
-- p_is_admin_override let a tenant_admin bypass the assigned-step check, and NO
-- layer rejected p_actor = approvals.requester_id. A person must never approve,
-- conditionally approve, hold, or reject a gate they themselves requested --
-- this holds even for system_admin / tenant_admin.
--
-- v5 changes ONLY the authorization guard (no argument-signature change, so this
-- is a CREATE OR REPLACE that preserves the execute grants): after locking and
-- loading the approval row, and BEFORE evaluating the current step, assignment,
-- or the admin-override boolean, RAISE if p_actor = v_approval.requester_id. The
-- override boolean can NEVER bypass this rule because the check runs first and is
-- unconditional. Everything else is identical to v4.
