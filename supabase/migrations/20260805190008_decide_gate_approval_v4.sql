-- GM-G3: decide_gate_approval v4 -- canonical phase-gate identity for signatures
--
-- v3 keyed the gate-approval signature row to entity_id = approvals.id. That is
-- the WRONG canonical identity: a gate signature is an endorsement of a specific
-- phase_gates row (project + gate number), and the audit/read path should be
-- keyed to that row so a signature can never be looked up by, or attached to, an
-- approval id that is not the gate's own governed identity.
--
-- v4 changes ONLY the signer identity binding (no argument-signature change, so
-- this is a CREATE OR REPLACE that preserves the execute grants):
--
--   * Resolve and LOCK exactly one phase_gates row for the approval's project and
--     gate number BEFORE inserting the signature. If zero or many exist, RAISE
--     and roll the whole decision back (the endorsement cannot be bound to an
--     ambiguous/absent gate identity).
--   * Insert the signature with entity_id = phase_gates.id (the canonical gate
--     identity) and project_id = the tenant-owned project (unchanged).
--
-- Everything else (atomic step/gate/phase writes, condition insertion, the
-- signature-required-to-endorse rule, multi-level progression) is identical to
-- v3.
...
BEGIN
  ...
  -- =========================================================================
  -- CANONICAL PHASE-GATE IDENTITY (endorsements only): resolve + LOCK exactly
  -- one phase_gates row for this project + gate number. The gate signature is
  -- bound to THIS row's id, never to the approval id. Require exactly one row
  -- so the endorsement can never be attached to an ambiguous/absent gate
  -- identity -- if the count is wrong, roll the whole decision back.
  -- =========================================================================
  ...
  -- current phase in_review -> approved (exactly one). Reuse the locked row id.
